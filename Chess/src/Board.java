...
void move(Tile prev, Tile next, Game g) {
	  ...
	  if (q != null) {
	    ...
	    if (p instanceof King) {
	    	...
	    	this.kingsButton[currTurn % 2] = next;
	    }
		  if (q instanceof King && g.getTakeMeEnabled()) {
			  Piece k = null;
			  for (Piece piece : this.pieces[(currTurn + 1) % 2]) {
			  ...
			  if (k == null)
				  this.kingsButton[(currTurn + 1) % 2] = (currTurn % 2 == 0) ? g.getBoard().tiles[0][0] : g.getBoard().tiles[7][7];
			  else
				  this.kingsButton[(currTurn + 1) % 2] = this.tiles[k.getPosition().getY()][k.getPosition().getX()];
		  }
	  }
	  else {
	    if (p instanceof Pawn) {
...
void takeMePath(Color turnColor, Game g) {
	  ...
	  if (!g.getBoard().take.isEmpty()) {
	  for (Piece p : this.pieces[colIndex]) {
	    if (p != enPass) {
	    if (g.getBoard().take.get(p) != null) {
	      g.getTakeCyan().add(this.tiles[p.getPosition().getY()][p.getPosition().getX()]);
	  ...
	  if (g.getBoard().take.get(enPass) != null) {
	    ...
	    g.getTakeCyan().add(this.tiles[y][x]);
...
void checkMate(Color color, Game g) {
	  ...
	  if (!isCheckPossible(g) && !g.getGameEnded()) {
	    ...
	    g.setGameEnded(true);
	      ...
	      if (this.pieces[colIndex].isEmpty()) {
	    	g.getMoveList().add("Game ended!");
	        JOptionPane.showMessageDialog(null, "No pieces left!  " + ((colIndex == 0) ? "White ": "Black ") + "wins!", "Ran out of pieces!", JOptionPane.ERROR_MESSAGE, Constants.images[colIndex][Constants.rand.nextInt(Constants.images[colIndex].length)]);
	        g.setGameEnded(true);
	      }
	      else {
	    	  if (largestPath <= 1) {
	    		  g.getMoveList().add("Game ended!");
	    		  JOptionPane.showMessageDialog(null, "No possible moves left!  " + ((colIndex == 0) ? "White ": "Black") + "wins!", "Ran out of moves!", JOptionPane.ERROR_MESSAGE, Constants.images[colIndex][Constants.rand.nextInt(Constants.images[colIndex].length)]);
	    		  g.setGameEnded(true);
	    	  }
	      }
	  }
