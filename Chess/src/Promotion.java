...
public Promotion(Color c, Tile t1, Game g, Gamewindow gw) {
	  ...
	  if (g.getAilevel() == 3 || (c.equals(Constants.colors[g.getAiColor()]) && g.getAilevel() > 0)) {
		  ...
		  if (g.getAilevel() == 3 && (g.getAiColor() != g.getTurnCount() % 2)) {
			  // TODO attempt to fix a glitch that causes a blank minimized popup to occur
			  // when aiColor != currcolor's turn
			  // Doesn't seem to affect gameplay even with the window open
			  g.setAiColor((row + 1) % 2);
		  }
