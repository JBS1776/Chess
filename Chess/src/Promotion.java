...
public Promotion(Color c, Tile t1, Game g, Gamewindow gw) {
	  ...
	  if (g.getAilevel() >= 3 || (c.equals(Constants.colors[g.getAiColor()]) && g.getAilevel() > 0)) {
		  ...
		  gw.setEnabled(true);
	      g.getBoard().getPieces()[row].remove(tile.getPiece());
	      g.getCapturedPieces().get(tile.getPiece().findIndex(Constants.colors[row])).add(tile.getPiece());
	      tile.setPiece(null);
	      tile.setImage(null);
	      t1.setIcon(null);
	      tile.setPiece(piece);
	      ...
	      g.getBoard().getPieces()[row].add(piece);
	      ...
	      if (!g.getTakeMeEnabled()) {
	        g.getBoard().getKingButton()[row].setBackground(Constants.TURNHIGHLIGHT);
	               if (g.getBoard().isKingInCheck(g.getTurnColor(), g)) {
	                 g.getBoard().getKingButton()[row].setBackground(Constants.CHECKHIGHLIGHT);
	               }
	    ...
	    public void actionPerformed(ActionEvent e) {
	     ...
	     if (source instanceof PieceButton) {
	      ...
	      if (piece instanceof King) {
	       ...
	       //g.getBoard().getKingButton()[g.getTurnCount() % 2] = t1;
	       gw.setEnabled(true);
	      }
	      ...
	      if (promPiece instanceof King && g.getTakeMeEnabled())
	    	  g.getBoard().getKingButton()[(g.getTurnCount() + 1) % 2] = t1;
	      String str = g.getMoveList().get(g.getMoveList().size() - 1);
